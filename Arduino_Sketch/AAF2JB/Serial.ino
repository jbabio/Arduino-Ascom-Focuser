...
void BTSerialEvent() {
  while (BTSerial.available()) {
    ...
    if (inChar == '#')                       // if incoming character is hash, set flag so doCommand can action
      stringComplete = true;
  }
}
//-------------------------------------------------------------------------------
...
//------------------------------------------------------------------
// Do commands incoming from BT serial port
//------------------------------------------------------------------
    ...
    case 'T': // Set Target Position
      {
        ...
        unsigned int targetPosI = targetPosS.toInt();
        targetPosition = targetPosI;
        BTSerial.print("T" + targetPosS + ":OK#");
        break;
      }
    case 'C': // Get Temperature
      {
    ...
    case 'I': // Set Initial Position. Sets Position without any movement
      {
        ...
        currentPosition = initPosI;
        targetPosition = initPosI;
        BTSerial.print("I" + initPosS + ":OK#");
        break;
      }
    case 'P': // Get Current Position
      {
        String currentPositionS = String(currentPosition);
        BTSerial.print("P" + currentPositionS + ":OK#");
        break;
      }
    case 'H': // Halt
      {
        targetPosition = currentPosition;
        String currentPositionS = String(currentPosition);
        BTSerial.print("H" + currentPositionS + ":OK#");
        break;
      }
    case 'M': // Is motor moving
      {
        if (isMoving) {
          BTSerial.print("M1:OK#");
        } else {
          BTSerial.print("M0:OK#");
        }
        break;
      }
    case 'V': // Get Version and abilities
      {
        ...
        BTSerial.print( programName + " V" + programVersion + tempInstalled + posValid + "#");
        break;
      }
    default:
      {
